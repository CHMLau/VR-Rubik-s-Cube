window.onload = (event) =>{
  ...
  const poster1 = document.getElementById("js--poster1");
  ...
  for (let i = 0; i < places.length; i++){
    places[i].addEventListener('click', function(evt){
      ...
      att.value = "property: position; easing: linear; dur: 5000; to: " + this.getAttribute("position").x + " 1.6 " + this.getAttribute("position").z;
  ...
  fetch("https://en.wikipedia.org/w/api.php?action=parse&page=Rubik%27s_Cube&prop=wikitext&formatversion=2&format=json")
    ...
    .then((response) => {
      ...
      poster1.setAttribute("value", winnerslist[1]);
    });
